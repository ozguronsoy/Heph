...
namespace Heph
{
    ...
    public:
        ...
        constexpr BufferIterator operator+(size_t i) const
        {
        ...
        constexpr BufferIterator& operator+=(size_t i)
        {
        ...
        static constexpr reference Get(pointer& ptr, const Enum<BufferFlags>& flags, const buffer_size_t& bufferSize, const buffer_size_t& strides, const auto... indices)
            requires (sizeof...(indices) > 0 && sizeof...(indices) <= NDimensions && (std::is_convertible_v<decltype(indices), size_t> && ...))
        {
            ...
            {
                const size_t index = std::get<0>(std::forward_as_tuple(static_cast<size_t>(indices)...));
                return BufferIterator::Get<CheckErrors>(ptr, flags, bufferSize, strides, index);
            }
            else
            {
